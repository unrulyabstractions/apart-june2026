...
def _permutation_signature(s: SesgoSample) -> str:
    """Label-style-independent signature of the role->position ordering.

    The rendered prompt lists three "<marker> <option text>" lines; dropping the
    marker leaves the option texts in displayed order, i.e. the permutation.
    """
    opts: list[str] = []
    ...
    return " | ".join(opts[-3:]) if len(opts) >= 3 else s.prompt_text[-120:]


def _axis_key(s: SesgoSample, axis: str) -> str:
    """The value of one format axis (label_style marker string or permutation)."""
    return s.label_style if axis == "label_style" else _permutation_signature(s)
