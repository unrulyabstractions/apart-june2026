...
def _batch_two_option(
    samples: list[SesgoPromptSample],
    runner: TernaryChoiceRunner,
    config: SesgoQueryConfig,
) -> list[SesgoTwoOption | None]:
    """2-option forced-choice (target vs other) per prompt; None when disabled.

    A single cheap choose2 per prompt over a BinaryChoiceRunner wrapping the SAME
    loaded weights (the expensive choose3/generation stays batched). Mirrors the
    single-sample path's _non_thinking_2opt.
    """
    if not config.do_two_option:
        return [None] * len(samples)
    binary = BinaryChoiceRunner.from_runner(runner)
    out: list[SesgoTwoOption | None] = []
    for s in samples:
        choice = binary.choose2(s.text_2opt, _prefix(s), s.option_labels_2opt)
        out.append(SesgoTwoOption.from_binary(choice, s.position_labels_2opt))
    return out
...
def query_chunk(
    ...
    return [
        SesgoSample(
            ...
            context_condition=s.context_condition,
            language=s.language,
