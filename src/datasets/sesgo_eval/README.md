...
| **Thinking** | Sample `n_thinking_samples` free-form generations (`temperature > 0`, via `generate_with_entropy`), parse which role each chose. | A `SesgoThinking`: per-role `mean` (pick fraction) and `std` (population std of the one-hot indicator), `sample_size` (#parsed, may be 0), `predicted`, plus `vocab_entropies` — one mean next-token Shannon entropy (nats) PER draw (over ALL draws) with `mean_vocab_entropy` / `std_vocab_entropy` summaries. |
