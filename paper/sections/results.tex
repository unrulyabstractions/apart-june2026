...
family, and the largest models (\texttt{Qwen3-32B}, \texttt{Llama-3.1-70B}) reach
${\approx}1.0$ (\Cref{fig:results-stability}). The two axes can diverge: for several
mid-size models a label swap is far more disruptive than a position swap (e.g.\
\texttt{Mistral 8B}, order $0.85$ vs.\ label $0.27$), so cosmetic relabelling alone can
...
\plotfig{figures/combined_invariance_old_new.png}{combined_invariance_old_new.png}{0.92\linewidth}{\textbf{Smaller models do not answer consistently.} Fraction of items whose committed answer is unchanged when the option \emph{positions} are swapped (left, order stability) and when the \texttt{a)b)c)} \emph{labels} are swapped (right, label stability), plotted vs.\ model size for both the earlier stability models (filled squares, probability readout) and the new sweep (open circles non-thinking, filled triangles thinking; greedy-readout choice), Wilson $95\%$ CIs, $n$ items annotated. Both axes rise with scale across families and the largest models reach ${\approx}1.0$. For several mid-size models a label swap is far more disruptive than a position swap.}{fig:results-stability}
