...
# SSH_EPHEMERAL_OPTS (from _ssh_target.sh) bounds the wait_ssh readiness probe (a
# freshly-'running' box whose sshd is not up yet fails FAST), tears down a stalled
# session, and — crucially at fleet scale — discards host keys so a reused Vast proxy
# endpoint never hangs as "waiting for sshd" (gotcha #1).
