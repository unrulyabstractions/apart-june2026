...
## ⭐ Seamless job-based fleet (CANONICAL — use this for many boxes)

The flow below (`fleet_jobs_run.sh`) is the canonical way to run a fleet. It is built
around four guarantees, each enforced by construction, not by remembering to check:

| Requirement | How it is guaranteed |
|-------------|----------------------|
| **100+ boxes, seamless** | `fleet_state_daemon.sh` polls ALL instances ONCE per cycle into a shared cache (`.fleet/instances_state.json`); every box reads status/SSH from the cache (one file read) instead of its own paginated API walk. The SSH endpoint is resolved once per box and inherited by `sync_up`/`at_vast`. API load is ~3 calls/cycle regardless of fleet size. A bandwidth semaphore (`BW_CONCURRENCY`) caps only the local sync steps; the GPU collects all run at once. |
| **Every job ALWAYS monitored** | `fleet_monitor_agent.sh` is auto-started by the orchestrator. It renders a live dashboard (`.fleet/monitor.dash`) and **force-verifies** each job the instant its output lands — running `scripts/verify_experiment_output.py` and writing `VERIFIED`/`BROKEN` to the manifest. You cannot run an unmonitored fleet. |
| **No override / merge issues** | Each box is a **job** with an id; its output lands ONLY in `jobs/<job-id>/output/` (gitignored), never `out/`. Promotion to `out/` is the separate, manual, **verified-only** step `promote_verified_jobs.sh`, which re-verifies the payload (ignores the manifest's claim), refuses anything not `VERIFIED`, and reports every existing-file conflict instead of clobbering. |
| **Nothing silently fails, EVER** | Every job ends in a terminal manifest state — `promoted`/`verified`/`broken`/`failed(reason)`. Every abort path calls `jr_fail` (recorded + announced). The monitor **reconciles** at the end and screams about any job not terminal (a box that died without recording is a loud failure, never silence). |

### The new scripts

| Script | Role |
|--------|------|
| `job_registry.sh`        | per-job id + `jobs/<id>/` + `manifest.json` (status/verdict/instance/timestamps). Refuses to create a mislabelled job (empty model/study). |
| `fleet_state_daemon.sh`  | one shared poller → `.fleet/instances_state.json` (atomic; keeps last-good on API flap). |
| `fleet_state_lookup.sh`  | `cached_instance_status` / `cached_ssh_target` — cache-first, live single-id fallback. |
| `fleet_monitor_agent.sh` | always-on dashboard + force-verify each synced job + end-of-run reconciliation. |
| `fleet_jobs_run.sh`      | the orchestrator: starts daemon+monitor, creates a job per box, runs all boxes concurrently, records every failure loudly. **One study per run.** |
| `promote_verified_jobs.sh` | the manual, verified-only move from `jobs/<id>/output/` → `out/`. |

### The exact commands

```bash
# 1. Launch the boxes (spends money) — writes cloud/.fleet/<tag>.id + <tag>.job
bash cloud/fleet_launch.sh

# 2. Drive the whole fleet, monitored + verified, ONE study per run:
STUDIES=forking bash cloud/fleet_jobs_run.sh
#   watch it live in another shell:   watch -n2 cat cloud/.fleet/monitor.dash
#   transitions + failures:           tail -f cloud/.fleet/monitor.log

# 3. After it returns, MANUALLY promote — only VERIFIED payloads move, nothing clobbers:
bash cloud/promote_verified_jobs.sh             # dry-run: what would promote + every refusal
bash cloud/promote_verified_jobs.sh --promote   # actually move verified payloads into out/
```

The deterministic monitor is the unskippable gate; for the deep adversarial pass (VIEW
each figure, cross-check intent), additionally spawn the `verifier` subagent on the
promoted outputs before treating any result as done.

> The older `fleet_run.sh` (and its `fill_fleet_driver.sh` / `fleet_big_retry.sh`
> chains) still exist for multi-study-per-box runs and are unchanged. New work should
> prefer the job-based flow above.
...
### Full-data baseline (`full_data`): all languages × all origins × scaffolds

The full SESGO grid (es+en × original+BBQ-adapted × {none + 3 scaffolds} = 24480
prompts) runs as the `full_data` study on a heavily-sharded RTX 4090 fleet. It
writes to its OWN `out/sesgo/full_data/` tree, so it never clobbers the
es-original runs:

```bash
FLEET_DIR=cloud/.fleet_fulldata FLEET_MODELS=Qwen/Qwen3-0.6B FLEET_SHARDS=16 \
  bash cloud/fleet_launch.sh
# BATCH_SIZE=16 + HF_FORWARD_MICRO_BATCH=8 keep the teacher-forced forward within a
# 24 GB 4090 (the scaffolded prompts are long; a 64-wide batch OOMs).
FLEET_DIR=cloud/.fleet_fulldata STUDIES=full_data GENERATE_ALL_DATA=1 \
  BATCH_SIZE=16 HF_FORWARD_MICRO_BATCH=8 \
  bash cloud/fleet_run.sh    # generate full grid on-box, run, sync-back, self-destruct

# After the fleet self-destructs, fold the 16 shard slices into one dataset:
uv run python sesgo/baseline/combine_full_data_shards.py Qwen3-0.6B   # -> out/sesgo/full_data/Qwen3-0.6B/
uv run python sesgo/baseline/visualize_full_data_samples.py out/sesgo/full_data/Qwen3-0.6B/response_samples.json
```
