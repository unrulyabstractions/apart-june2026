...
#
# Writes a run log to cloud/.sel_<TAG>.log and the instance id to
# cloud/.sel_<TAG>.iid (so a watcher can find/destroy it out-of-band if needed).
...
RUN_ENV="export HF_TOKEN='${HF_TOKEN:-}' HF_FORWARD_MICRO_BATCH='${HF_FORWARD_MICRO_BATCH:-}' HF_GEN_MICRO_BATCH='${HF_GEN_MICRO_BATCH:-}' PYTORCH_CUDA_ALLOC_CONF='${PYTORCH_CUDA_ALLOC_CONF:-}' MODEL='$MODEL' STUDIES='selection' N_THINKING='$N_THINKING' MAX_NEW_TOKENS='$MAX_NEW_TOKENS' BATCH_SIZE='$BATCH_SIZE' SUBSAMPLE='$SUBSAMPLE'"
# Start detached; write a done-marker ($?-stamped) the poller watches for.
run_on_box "cd $REMOTE_ROOT; rm -f sel_run.done sel_run.log; setsid bash -c \"$RUN_ENV; bash cloud/fleet_model_run.sh > sel_run.log 2>&1; echo \\\$? > sel_run.done\" >/dev/null 2>&1 & echo started" \
  2>&1 | sed "s/^/[$TAG run] /"

RESULT_JSON="out/sesgo/selection/$BARE_MODEL/response_samples.json"
...
for i in $(seq 1 300); do
  sleep 15
  DONE="$(run_on_box "cat $REMOTE_ROOT/sel_run.done 2>/dev/null" 2>/dev/null | tr -dc '0-9')"
...
if [ "$RC" != "0" ]; then
  ...
  run_on_box "tail -20 $REMOTE_ROOT/sel_run.log" 2>&1 | sed "s/^/[$TAG runlog] /"
  exit 7
fi
log "collection finished cleanly (rc=0)"

# ── 8. Verify the result is non-empty BEFORE we destroy the box ──
NS="$(run_on_box "[ -x .venv/bin/python ] && .venv/bin/python -c \"import json; print(len(json.load(open('out/sesgo/selection/$BARE_MODEL/response_samples.json'))['samples']))\" 2>/dev/null || echo 0")"
...
run_on_box ".venv/bin/python sesgo/selection/visualize_selection_samples.py out/sesgo/selection/$BARE_MODEL/response_samples.json" \
  2>&1 | sed "s/^/[$TAG viz] /"
...
SYNC_SUBDIR="$TAG" STUDIES="selection" INSTANCE="$INSTANCE" bash "$HERE/sync_back.sh" 2>&1 | sed "s/^/[$TAG back] /"
