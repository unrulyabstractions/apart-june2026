...
Run-by-path driver, STAGE 5 of the forking-paths study. Renders the 5-panel
forking-dynamics figure: the STACKED-AREA O_t-vs-token-position headline (one
colored band per outcome category, y in [0,1]) with the change-point token marked;
a TOKEN STRIP beneath that highlights the forking tokens (base-path tokens where
O_t jumps abruptly — the words that flip the answer); the pull/drift/potential
dynamic states; a single diversity line H(O_t); and the survival + change-point
posterior. Reads the two JSONs the capture + analysis drivers wrote; writes PNGs
alongside them.
...
from experiment.forking.forking_plot_styles import (  # noqa: E402
    OUTCOME_COLORS,
    save_fig,
    titled,
    token_strip,
)
from experiment.forking.render_branching_tree import plot_branching_tree  # noqa: E402
...
def _fork_flags(jumps: list[float]) -> list[bool]:
    """Flag base-path tokens whose O_t jump Delta_t is abruptly large.

    A token forks when its jump exceeds mean + 1 std of the jump series — the
    sharp first-difference spikes that flip the outcome distribution. Falls back
    to no flags on a degenerate (empty / flat) series.
    """
    arr = np.asarray(jumps, dtype=float)
    if arr.size == 0 or arr.std() == 0:
        return [False] * arr.size
    threshold = arr.mean() + arr.std()
    # Skip t=0: its jump is just the baseline transition off the prior o_0, not a
    # within-path fork — only later abrupt spikes are answer-flipping words.
    return [bool(i > 0 and v > threshold) for i, v in enumerate(arr)]
...
def _stacked_area(ax, traj: ForkingTrajectory, cp) -> None:
    ...
    if cp.forking_token_index >= 0:
        ax.axvline(cp.forking_token_index, color="red", lw=2.2, ls="--", label="fork")
    ax.set_ylim(0, 1)
    ax.set_xlim(0, max(xs[-1], 1))
    ax.set_ylabel("O_t")
    ax.legend(loc="upper left", fontsize=7, ncol=2, framealpha=0.85)
    titled(ax, "Outcome O_t")


def _states_panel(ax, st) -> None:
    """Pull / drift / potential magnitudes vs token position."""
    xs = np.arange(len(st.pull))
    ax.plot(xs, st.pull, "-o", ms=3, color="#0072B2", label="pull")
    ax.plot(xs, st.drift, "-o", ms=3, color="#D55E00", label="drift")
    ax.plot(xs, st.potential, "-o", ms=3, color="#009E73", label="potential")
    ax.set_ylabel("magnitude")
    ax.legend(fontsize=7)
    ax.grid(alpha=0.25)
    titled(ax, "Dynamic states")


def _diversity_panel(ax, dv) -> None:
    """Single diversity line = Shannon entropy H(O_t) vs token position."""
    xs = np.arange(len(dv.balance))
    ax.plot(xs, dv.balance, "-o", ms=3, color="#CC79A7", label="diversity")
    ax.set_ylabel("H(O_t)")
    ax.legend(fontsize=7)
    ax.grid(alpha=0.25)
    titled(ax, "Diversity")
...
def _survival_tau_panel(ax, sv, cp) -> None:
    ...
    ax.plot(xs, sv.survival, "-o", ms=3, color="#0072B2", label="survival")
    ax.bar(xs, sv.hazard, color="#D55E00", alpha=0.35, label="hazard")
    tau = cp.tau_posterior
    ax.plot(np.arange(len(tau)), tau, "-", color="red", lw=1.6, label="p(τ)")
    ax.set_ylim(0, 1.02)
    ax.set_xlabel("token position t")
    ax.set_ylabel("prob")
    ax.legend(fontsize=7)
    ax.grid(alpha=0.25)
    titled(ax, "Survival + change-point")
...
def _figure(traj: ForkingTrajectory, analysis: ForkingAnalysis):
    ...
    jumps = analysis.dynamic_states.forking_magnitude
    ...
    token_strip(fig.add_subplot(gs[1, :]), traj.base_token_texts, jumps, _fork_flags(jumps))
