...
## Sharded capture — three drivers, N+1 boxes

For long base paths the single-box capture is split across `N` cloud boxes by
POSITION, run by the orchestrator `cloud/run_forking_sharded_fleet.sh`:

- **Phase 1 (`decode_forking_base_path.py`, one box)** — decode the base path ONCE
  and enumerate every `(t, w)` branch prefix; serialize the whole plan to
  `base_path.json`. No forking happens here (the cheap phase).
- **Phase 2 (`collect_forking_shard.py`, N boxes in parallel)** — box `k` loads
  `selected_item.json` + `base_path.json` and forks ONLY its slice
  `[t for t in range(P) if t % N == k]`, writing `forking_shard_<k>_of_<N>.json`.
  ONLY shard 0 also computes the prior `o_0`. Each shard preserves the REAL index
  `t` on every `ForkPosition`, so nothing is renumbered.
- **Phase 3 (`merge_forking_shards.py`, local)** — concatenate all shards' positions
  by real `t`, validate 0..P-1 is covered exactly once (gaps/duplicates logged
  LOUDLY, never silently dropped), and rebuild one `forking_trajectory.json` — a
  drop-in for the existing analyze + plot drivers.

The output is identical in schema to single-box capture, so Stages 4-5 are unchanged.
...
## Scaffold vs no-scaffold comparison (`plot_forking_comparison.py`)

The forking pipeline doubles as a debiasing-scaffold *dynamics probe*: run the SAME
ambiguous item with a debiasing scaffold prepended and again without, and ask whether
the scaffold collapses the outcome distribution onto "unknown" (abstention) earlier or
more strongly. Three optional `select_forking_item.py` flags make the two conditions a
fair, single-item comparison:
- `--force-question-id <qid>` skips the entropy pilot entirely and selects the one
  ambiguous prompt with that `question_id` — NO model/GPU is needed for selection, so
  both conditions are guaranteed to fork the IDENTICAL item (only the preamble differs).
  Its pilot record is an all-zero placeholder (`entropy 0`, `n_parsed 0`).
- `--scaffold <scaffold_id>` looks up the matching `Scaffold` in `sesgo.scaffolds` and
  generates the prompt with `include_no_scaffold=False` + `generate(items, [scaffold])`,
  so the rendered `text` carries the bilingual preamble and the sample's `scaffold_id`
  equals the id. With no `--scaffold` the baseline no-scaffold prompt is rendered.
- `--run-tag <suffix>` suffixes the bare-model out subdir
  (`out/sesgo/forking/<bare-model><suffix>/`) and is threaded into `shard_out_dir`, so
  the conditions write DISJOINT paths. The same `--run-tag` is added to `collect` /
  `analyze` / `plot` so every stage reads/writes the right condition's dir. Default
  `""` preserves all existing single-condition paths exactly.

`plot_forking_comparison.py` then loads the two `forking_trajectory.json` files and
renders ONE figure (no suptitle/prose): the baseline stacked-area `O_t` on the top row,
the scaffold stacked-area `O_t` on the middle row (small corner labels "No scaffold" /
"Interpretive-direction scaffold"), and a bottom row overlaying both conditions' answer
diversity `H(O_t)` and abstention (`unknown`) probability mass — so the reader sees
whether the scaffold locks onto "no se sabe" earlier/more. Colors reuse `OUTCOME_COLORS`
from `forking_plot_styles.py`; output is
`out/sesgo/forking/Qwen3-0.6B_scaffold_vs_baseline.png`.

`cloud/run_one_forking_box_32b.sh` exposes the same three knobs as passthrough env vars
(`SELECT_SCAFFOLD`, `SELECT_FORCE_QID`, `RUN_TAG`), forwarded to every on-box stage;
`--scaffold`/`--force-question-id` are appended only when their env is non-empty, and
`--run-tag=$RUN_TAG` (the `=`-form, to survive a leading-`-` suffix) is always passed.
The tagged out subdir flows through the verify/report/sync-back steps, so two boxes with
disjoint `RUN_TAG`s capture the two conditions in parallel and quarantine separately.
All three empty == the original full-pilot single-box behaviour.
